...
fn minimized_replay_should_decode_all_frames() {
    ...
    for c in &chunks { combined.extend_from_slice(c); }
    ...
    for c in chunks {
        ...
        loop {
            ...
            match dec.decode(&mut buf) {
                Ok(Some(StompItem::Frame(f))) => { eprintln!("decoded frame, remaining buf len={}", buf.len()); decoded += 1; bodies.push(f.body); }
                Ok(Some(StompItem::Heartbeat)) => { eprintln!("decoded heartbeat"); }
                Ok(None) => { eprintln!("decode returned None (need more bytes)"); break; }
    ...
    assert_eq!(decoded, expected, "decoder must decode all frames in minimized sequence");
}
