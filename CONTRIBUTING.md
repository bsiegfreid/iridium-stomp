...
- Use the included unit tests that feed frames in small increments and random splits. Key tests:
	- `cargo test --manifest-path iridium-stomp/Cargo.toml minimized_regression -- --nocapture`
	- `cargo test --manifest-path iridium-stomp/Cargo.toml regression_replay -- --nocapture`
	- `cargo test --manifest-path iridium-stomp/Cargo.toml -- --test-threads=1` (optional: serial run for deterministic output)
